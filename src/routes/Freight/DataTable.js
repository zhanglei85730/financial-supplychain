...
function DataTable({
  ...
  },
  {
    ...
    render: (value, row, index) => {
      if (row.hasOwnProperty('items') &&
        Array.isArray(row.items) &&
        row.items.length > 0 &&
        row.items[0].hasOwnProperty('relateOrderId')) {
        return row.items[0].relateOrderId;
      } else {
        return '';
      }
    },
  ...
  return (
    <div>
      ...
      <Table
        ...
        scroll={Object.assign(consts.globalTableScroll, { x: 2764 })}
        pagination={{
          total,
