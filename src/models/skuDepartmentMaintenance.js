...
import * as service from '../services/skuDepartmentMaintenance.js';
...
export default {
  namespace: 'skuDepartmentMaintenance',
  state: {
    formCollapse: false,
    list: [],
    loading: true,
  ...
  effects: {
    ...
    * deleteData({ payload }, { call, put }) {
      const response = yield call(service.deleteData, payload);
    },
  },
  subscriptions: {
    setup({ dispatch, history }) {
      ...
      return history.listen(({ pathname }) => {
        if (pathname === '/baseData/skuDepartmentMaintenance' || pathname === '/') {
