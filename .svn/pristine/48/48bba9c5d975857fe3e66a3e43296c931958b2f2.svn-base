...
export function getToken() {
  // return getSecurityHeaderFormLocal().then((securityHeader) => {
  //   if (securityHeader.hasOwnProperty('headers') && securityHeader.headers.hasOwnProperty('security-header')) {
  //     const token = securityHeader.headers['security-header'];
...
// table分页点击事件
export function changePagination(page, pageSize, dispatch, modleNameSpace, params) {
...
// form参数格式化,多个条件用,隔开如：“a,b”
export function formParamsFormater(fieldsValue) {
    ...
    if (fieldsValue[key] !== undefined) {
      if (Array.isArray(fieldsValue[key])) {
        ...
        if (fieldsValue[key].length === 2 &&
          ...
          params[key] = `${fieldsValue[key][0].format('YYYY-MM-DD 00:00:00')},${fieldsValue[key][1].format('YYYY-MM-DD 00:00:00')}`;
          // 日期 非时间段
        } else if (fieldsValue[key].length === 1 &&
          fieldsValue[key][0].hasOwnProperty('_isAMomentObject') &&
          fieldsValue[key][0]['_isAMomentObject']) {
          params[key] = `${fieldsValue[key][0].format('YYYY-MM-DD 00:00:00')}`;
          // 其他
        } else {
