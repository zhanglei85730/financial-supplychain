import * as service from '../services/overseasTransferVerification.js';
import * as util from '../utils/util.js';
import consts from '../config/const.js';

export default {
  namespace: 'overseasTransferVerification',
  state: {
    formCollapse: false,
    list: [],
    loading: true,
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      ...
      return history.listen(({ pathname }) => {
        if (pathname === '/overseasTransfer/verification' || pathname === '/') {
