...
import * as service from '../services/fbaTransferAccount.js';
import * as util from '../utils/util.js';
import consts from '../config/const.js';
import { validateFetch } from '../services/common.js';

export default {
  namespace: 'fbaTransferAccount',
  state: {
    formCollapse: false,
    list: [],
    loading: true,
  ...
  reducers: {
    ...
    query(state, { payload }) {
      ...
      return { ...state, list: [...sumPageList], total: payload.total, loading: false };
    },
    // 选择的数据行数
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      ...
      return history.listen(({ pathname }) => {
        if (pathname === '/FBATransfer/account' || pathname === '/') {
